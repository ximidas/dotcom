        ...
        html! {
            <nav class="pagination is-right" role="navigation" aria-label="pagination">
                <ul class="pagination-list">
                    { self.view_pages_buttons(ctx.props()) }
                </ul>
            </nav>
        }
    }
...
impl Pagination {
    ...
    fn view_links(&self, props: &Props) -> Html {
        ...
        let pages_next = total_pages.saturating_sub(page) as usize;
 ...
 fn view_pages_buttons(&self, props: &Props) -> Html {
        let Props {
            page,
            total_pages,
            route_to_page: to,
        ...
        html! {
            <>
                { (!prev_disabled).then(|| {
                    html! {
                        <li><Link<Route, PageQuery>
                            classes={classes!("pagination-arrow")}
                            query={Some(PageQuery{page: page - 1})}
                            to={to.clone()}
                        >
                            { "Previous" }
                        </Link<Route, PageQuery>></li>
                    }
                })}
                { self.view_links(props) }
                { (!next_disabled).then(|| {
                    html! {
                        <li><Link<Route, PageQuery>
                            classes={classes!("pagination-arrow")}
